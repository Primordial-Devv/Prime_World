RegisterNetEvent(Config.InventoryPrefix .. ':server:SetInventoryData', function(fromInventory, toInventory, fromSlot, toSlot, fromAmount, toAmount, fromInventoryIsClothesInventory, toInventoryIsClothesInventory)
	local src = source
	...
	elseif fromInventory == 'crafting' then
		local itemData = Config.CraftingTables[key].items[fromSlot]
		...
		if hasCraftItems(src, itemData.costs, fromAmount) then
			...
			SendWebhook(Webhooks.crafting, 'Crafting item', 7393279, '**' .. GetPlayerName(src) .. '**\n**Crafted item in:** ' .. Config.CraftingTables[key].name .. '\n**Item crafted:** ' .. itemData.name .. '\n**Timer:** ' .. itemData.time * fromAmount .. '\n**Amount:** ' .. fromAmount)
			Debug('Started ' .. itemData.name .. ' crafting in ' .. Config.CraftingTables[key].name .. ' with a delay time of ' .. itemData.time * fromAmount .. ', quantity ' .. fromAmount)
		else
			TriggerClientEvent(Config.InventoryPrefix .. ':client:UpdatePlayerInventory', src, true)
